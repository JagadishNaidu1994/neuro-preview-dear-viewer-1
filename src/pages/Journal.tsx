...
const Journal = () => {
  ...
  const fetchJournals = async () => {
    try {
      const {
        data,
        error
      } = await (supabase as any).from("journals").select("*").eq("published", true).order("created_at", {
        ascending: false
      });
  ...
  return <div className="min-h-screen bg-[#F8F8F5]">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        ...
        {/* Articles Grid */}
        {journals.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {journals.map(journal => <Link key={journal.id} to={`/journal/${journal.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-shadow duration-300">
                <div className="aspect-[16/10] overflow-hidden">
                  <img src={journal.image_url || "https://images.pexels.com/photos/5946071/pexels-photo-5946071.jpeg?auto=compress&cs=tinysrgb&w=800"} alt={journal.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
                </div>
                <div className="p-8">
