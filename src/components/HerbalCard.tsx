...
const HerbalCard = ({
  ...
  return <div className="w-full h-[360px] [perspective:1200px] cursor-pointer" onClick={() => setFlipped(!flipped)}>
      <div className={`relative w-full h-full transition-transform duration-700 [transform-style:preserve-3d] ${flipped ? "[transform:rotateY(180deg)]" : ""}`}>
        {/* Front */}
        <div className="absolute w-full h-full [backface-visibility:hidden] rounded-xl shadow-md p-6 bg-slate-300">
          <div className="flex justify-between items-start mb-3">
            <span className="text-sm font-semibold text-[#161616]">[{herb.id}]</span>
            <span className="w-3 h-3 rounded-full" style={{
            backgroundColor: herb.color || "#C1A85F"
          }} />
          </div>
        ...
        {/* Back */}
        <div className="absolute w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] bg-[#F6F5EF] rounded-xl shadow-md p-6 text-sm text-[#444] overflow-hidden">
          ...
          <ul className="list-disc ml-5 text-blue-600 underline">
            {herb.back.studies.map((link, i) => <li key={i}>
